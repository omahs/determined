...
def http_connect_tunnel(sess: api.BaseSession, service: str) -> None:
    parsed_master = request.parse_master_address(sess._master)
    assert parsed_master.hostname is not None, f"Failed to parse master address: {sess._master}"
    ...
    url = request.make_url(sess._master, f"proxy/{service}/")
    ws = lomond.WebSocket(request.maybe_upgrade_ws_scheme(url), proxies=proxies)
    if isinstance(sess, api.Session):
        ws.add_header(b"Authorization", f"Bearer {sess._utp.token}".encode())
    ...
    c2 = threading.Thread(
        target=copy_from_websocket, args=(unbuffered_stdout, ws, ready_sem, sess._cert)
    )
    c1.start()
    c2.start()
    c1.join()
...
def _http_tunnel_listener(
    ...
    url = request.make_url(sess._master, f"proxy/{tunnel.service_id}/")
    ...
    class TunnelHandler(socketserver.BaseRequestHandler):
        def handle(self) -> None:
            ...
            c2 = threading.Thread(
                ...
                args=(self.request, ws, ready_sem, sess._cert),
            )
            c1.start()
            c2.start()
            c1.join()
