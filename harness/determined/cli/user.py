...
def activate_user(args: Namespace) -> None:
    user_obj = client.get_user_by_name(args.username)
    user_obj.activate()
...
def log_in_user(args: Namespace) -> None:
    ...
    utp = authentication.login(args.master, username, password, certs.cli_cert)
...
def log_out_user(args: Namespace) -> None:
    if args.all:
        authentication.logout_all(args.master, certs.cli_cert)
    else:
        # Log out of the user specified by the command line, or the active user.
        authentication.logout(args.master, args.user, certs.cli_cert)


@cli.login_sdk_client
def rename(args: Namespace) -> None:
    user_obj = client.get_user_by_name(args.target_user)
...
def link_with_agent_user(args: Namespace) -> None:
    ...
    user_obj = client.get_user_by_name(args.det_username)
...
def edit(args: Namespace) -> None:
    ...
    det = Determined._from_session(sess)
    user_obj = det.get_user_by_name(args.target_user)
    changes = []
